...
public class BitcoinServiceImplementation extends BaseService implements BitcoinService {
    ...
    public Optional<BitcoinTransactionOutputDTO> getBitcoinTransactionOutput(final String txId, final int vout) {
        ...
        if (!bitcoinTransactionOutputs.isEmpty()) {
            ...
            logger.info("getBitcoinTransactionOutput - The transaction {} is already in our database", txId);
            final Optional<BitcoinTransactionOutputDTO> output = bitcoinTransactionOutputs.stream()
                    ...
                    .findFirst();
            if (output.isPresent()) {
                logger.info("getBitcoinTransactionOutput - Output {} of the transaction {} is already in our database", vout, txId);
                return output;
            } else {
                logger.info("getBitcoinTransactionOutput - Output {} of the transaction {} is NOT in our database", vout, txId);
            }
        }
        ...
        logger.info("getBitcoinTransactionOutput - The transaction {} is NOT in our database, we call mempool", txId);
        ...
        if (transaction == null) {
            ...
            logger.info("getBitcoinTransactionOutput - Transaction {} is NOT in the blockchain", txId);
            ...
            if (vout >= transaction.getVout().size()) {
                logger.info("getBitcoinTransactionOutput - Transaction {} is in the blockchain but output {} doesn't exist", txId, vout);
                return Optional.empty();
            } else {
